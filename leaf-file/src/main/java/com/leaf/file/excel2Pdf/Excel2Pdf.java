    ...
    public void convert() throws DocumentException, MalformedURLException, IOException {
        ...
        if(this.objects.size() <= 1){
    		...
    		for(PdfPTable table : tableList){
    			...
    			getDocument().newPage();//.add(new AreaBreak(AreaBreakType.NEXT_PAGE));
    		}
        }
        //Multiple ones
        if(this.objects.size() > 1){
...
//      	table.setWidthPercentage(new float[]{100} , writer.getPageSize());
    ...
    protected void toCreateContentIndexes(PdfWriter writer , Document document , List<ExcelObject> objects) throws DocumentException{
        ...
        Font sheetNameFont = new Font(Resource.BASE_FONT_CHINESE , 10 , Font.ITALIC);
        sheetNameFont.setColor(new BaseColor(0,0,255));
        //
        for (int i = 0; i < objects.size(); i++) {
            ExcelObject o = objects.get(i);
            String text = (i+1)+" "+o.getAnchorName();
            ...
            //
            table.addCell(cell);
            List<Sheet> sheetList = o.getExcel().getSheetList();
            for(int j=0;sheetList!=null && j<sheetList.size();j++){
            	Sheet sheet = sheetList.get(j);
            	String sheetText = (j+1)+" "+sheet.getSheetName();
                Anchor sheetAnchor = new Anchor(sheetText , sheetNameFont);
                sheetAnchor.setReference("#" + sheet.getSheetName());
                //
                PdfPCell sheetCell = new PdfPCell(sheetAnchor);
                sheetCell.setBorder(0);
                //
                table.addCell(sheetCell);
            }
        }
        //
        document.add(table);
    ...
    protected void toCreateContentSheetIndexes(PdfWriter writer , Document document , List<String> sheetNames) throws DocumentException{
        PdfPTable table = new PdfPTable(1);
        table.setKeepTogether(true);
        table.getDefaultCell().setBorder(PdfPCell.NO_BORDER);
        //
